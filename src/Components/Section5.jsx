      ...
      <div className="h-full w-[99%] mx-auto flex flex-col lg:flex-row gap-2">
        ...
        <div className="w-full lg:w-[75%]">
          <div className="h-[10vh] w-full flex justify-between p-4 items-center bg-[#F3F4F6]">
            <p className="text-base sm:text-lg md:text-xl font-semibold">Tv's & Electronics</p>
            <p className="text-base sm:text-lg md:text-xl"  onClick={()=>navigate("/filter",{state:data?.data?.data[2]?.id})}>See All</p>
          </div>
  ...
  isLoading ? (
      ...
      <SliderSkeleton/>
          ...
          <Slider {...settings} className="w-full px-4">
            {data?.data?.data[2]?.product_deatils?.map((item, index) => (
              <div
                key={index}
        ...
        <div className="w-full lg:w-[24%] lg:block hidden">
          <img
      ...
      <div className="my-2 w-[99%] mx-auto">
        <img
