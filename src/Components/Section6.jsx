  ...
  return (
    <>
      <div className="h-full w-[99%] mx-auto flex flex-col lg:flex-row gap-4">
        ...
        <div className="w-full lg:w-[26%] lg:block hidden">
          <img
        ...
        <div className="w-full lg:w-[74%]">
          <div className="w-full flex justify-between p-4 items-center bg-[#F3F4F6]">
            <p className="text-lg sm:text-xl font-semibold">Laptop & Accessories</p>
            <p className="text-sm sm:text-base text-blue-600 cursor-pointer hover:underline"  onClick={()=>navigate("/filter",{state:data?.data?.data[3]?.id})}>See All</p>
          </div>
  ...
  isLoading ? (
      ...
      <SliderSkeleton/>
          ...
          <Slider {...settings} className="w-full px-4">
            {data?.data?.data[3]?.product_deatils?.map((item, index) => (
              <div
                key={index}
