...
const Section9 = () => {
  ...
  return (
    <>
      <div className="w-[99%] mx-auto flex flex-col lg:flex-row gap-4">
      
        <div className="w-full lg:w-[74%]">
          <div className="w-full flex justify-between p-4 items-center bg-[#F3F4F6]">
            <p className="text-lg sm:text-xl font-semibold">Mobile Accessories</p>
            <p className="text-sm sm:text-base text-blue-600 hover:underline cursor-pointer"  onClick={()=>navigate("/filter",{state:data?.data?.data[6]?.id})}>See All</p>
          </div>
  ...
  isLoading ? (
      ...
      <SliderSkeleton/>
          ...
          <Slider {...settings} className="w-full px-4">
            {data?.data?.data[6]?.product_deatils?.map((item, index) => (
              <div
                key={index}
                className="!w-[90%] sm:!w-[24vw] md:!w-[30vw] lg:!w-[18vw] xl:!w-[16vw] m-2 border border-gray-300 rounded-xl p-3 shadow-sm bg-white"
      ...
      <div className="my-4 w-[99%] mx-auto">
        <img
          src="https://crazzyhub.s3.amazonaws.com/media/banner/Mobile_Accessories.webp"
          alt="Mobile Accessories Banner"
